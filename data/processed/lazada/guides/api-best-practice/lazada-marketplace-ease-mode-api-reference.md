...
brand | Brand_id is optional if this field is used. | The brand name of the product, which can be queried by calling the GetBrandByPages API .
brand_id | If this field is used, the brand field is optional. | The product brand corresponds to the ID of the Lazada brand library, which can be queried by calling the GetBrandByPages API .
