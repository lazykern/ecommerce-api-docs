...
(b)      no API calls are made by the Application within a duration of 90 consecutive days, (referred to as a "**Dormant Application** ").
  ...
  * **Hibernation Mode:** All Applications under Hibernation Mode shall have Daily API Call Limit = 1.
...
4.2. For the purposes of imposing the following penalty measures, Lazada's assessment of the Developer's violation of the Developer Agreement, Lazada's Policies and Platform Rules are final, and Lazada may take one or more of the following measures against the Application or Developer depending on the circumstances:
  ...
  * **Warnings:** Lazada may issue a private or a public warning notification to the Developer for remediation of the violation within a reasonable time limit prescribed by Lazada.
  * **Stop  new seller authorization:**The Application cannot get authorization from new sellers. Sellers which have previously granted authorization can continue to use the Application before the current authorization expires but such sellers shall not be able to renew the authorization upon expiry of the authorisation.  
  * **Limit API calls:** Lazada may impose a limit on the API calls made by the Application. The API call limit for the Application is set to 1,000 calls per day (or such other limit as may be imposed by Lazada).
  * **Stop API calls:** Lazada may ban all API calls made by the Application.  
  * **Access to Lazada Content:** Lazada may suspend, limit or terminate access by the relevant Application or the Developer's other Applications to all or part of the Lazada Content (including Personal Data).
  * **Application offline:** The Application may be disabled, suspended or permanently removed, and once taken offline, such Application cannot call any APIs or be otherwise used or accessed by sellers.  
  * **Limit, Disable, Suspend or Terminate your Developer Account** : Lazada may limit, disable, suspend or terminate Account and/or any Application developed, operated or distributed by such Account or otherwise to impose conditions or revoke your access to the Account, Platform and/or any Tools. Once your Account is disabled, suspended or terminated: (a) all Applications under the Account will be taken offline and shall not be able to make any API calls; (b) Sellers will not be able to continue to use the Applications and the Application cannot get authorization from new sellers; (c) The Developer's identity on the Platform will be permanently frozen and/or removed, and Lazada will no longer cooperate with the Developer.
  * **Other Measures** : Other measures that are developed by Lazada Open Platform based on the needs of Platform management.